...
def parse_object_id(path: Path, fallback_idx: int | None = None) -> int:
    stem = path.stem
    ...
    raise ValueError(f"Cannot parse object id from '{stem}'")


def convert_fbx_to_obj(fbx_path: Path, obj_path: Path) -> Path:
...
def call_render(
    ...
    expected = len(np.load(pose_path)) * 2
...
def render(cfg) -> None:
    ...
    cad_paths = sorted(list(cad_dir.glob("*.ply")) + list(cad_dir.glob("*.obj")) + list(cad_dir.glob("*.fbx")))
    ...
    for idx, cad_path in enumerate(cad_paths):
        ...
        if cad_path.suffix.lower() == ".fbx":
            ...
            cad_path = convert_fbx_to_obj(cad_path, obj_converted_path)
    ...
    logger.info(f"Start rendering {len(final_cad_paths)} objects")
    ...
    values = list(
        tqdm(pool.imap_unordered(call_render_, range(len(final_cad_paths))), total=len(final_cad_paths))
    )
    correct_values = [val for val in values if val]
    finish_time = time.time()
    logger.info(f"Finished for {len(correct_values)}/{len(final_cad_paths)} objects")
    logger.info(f"Total time: {finish_time - start_time:.1f}s")
